...
void Animation::addAnimation(const AnimationBase& anim) // init animations
{
...
void Animation::update(float deltaTime) 
{
    ...
    if (timer >= currentAnimation->frameTime) 
    {
        ...
        if (currentFrame >= currentAnimation->frameCount) { currentFrame = currentAnimation->loop ? 0 : currentAnimation->frameCount - 1; }
    }
}

void Animation::render(Texture2D& texture, Vector2 position) const 
{
    if (!currentAnimation) return;
    ...
    Rectangle src = {(float)((frameIndex % columns) * frameWidth), 
