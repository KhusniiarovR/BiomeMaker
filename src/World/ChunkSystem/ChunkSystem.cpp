...
ChunkSystem::ChunkSystem(std::unordered_map<std::pair<int, int>, Chunk, PairHash>& chunks, const std::string& filename) 
...
void ChunkSystem::render(Renderer& renderer) const 
{
    ...
    // draw all tiles then all objects
    for (auto& [key, chunk] : chunks) { chunk.renderTiles(tilemap); }
    for (auto& [key, chunk] : chunks) { chunk.renderObjects(tilemap); }
}

void ChunkSystem::updateChunks(Vector2& playerPos) // load and unload chunks based on player position
{
    ...
    for (int y = loadStartY; y < loadEndY; ++y) 
    {
        for (int x = loadStartX; x < loadEndX; ++x) 
        {
            ...
            if (chunks.find(key) == chunks.end()) { chunks.emplace(key, Chunk(x, y, headers, worldFile)); }
        }
    }
    ...
    while (it != chunks.end()) 
    {
        ...
        if (chunkX < unloadStartX || chunkX >= unloadEndX || chunkY < unloadStartY || chunkY >= unloadEndY) 
        {
            if (it->second.getModified()) { overwriteChunk(chunkX, chunkY, it->second); }
            it = chunks.erase(it);
        } 
        else { ++it; }
    }
...
{
    if (cx < 0 || cy < 0 || cx >= numberOfChunks || cy >= numberOfChunks) 
    {
        mycerr << "Invalid chunk coordinates for overwrite: (" << cx << "," << cy << ")";
        return;
    }
    ...
    for (int y = 0; y < chunkSize; ++y) 
    {
        ...
        {
             biomeIds[y][x] = resolveBiomeId(chunk.getTiles()[y][x]); 
        }
    }
    ...
    writeObjectsChunk(objectBuffer, chunk.getObjects(), cx * chunkSize, cy * chunkSize, worldTileSize);
    ...
    ChunkHeader newHeader = { header.offsetBiome,  static_cast<uint32_t>(biomeData.size()), header.reservedSizeBiome, 
                              header.offsetObject, static_cast<uint32_t>(objectData.size()),header.reservedSizeObject};
...
void ChunkSystem::saveFullWorld() 
{
    ...
    for (int cy = 0; cy < numberOfChunks; ++cy) 
    {
        for (int cx = 0; cx < numberOfChunks; ++cx) 
        {
            ...
            if (chunkIt != chunks.end() && chunkIt->second.getModified()) 
            {
                ...
                for (int y = 0; y < chunkSize; ++y) 
                {
                    for (int x = 0; x < chunkSize; ++x) { biomeIds[y][x] = resolveBiomeId(chunk.getTiles()[y][x]); }
                }
                ...
                newHeaders[index] = { static_cast<uint32_t>(biomeOffset), biomeDataSize, biomeReserve, 
                                      static_cast<uint32_t>(biomeOffset) + biomeReserve, objectDataSize, objectReserve};
            } 
            else 
            {
                ...
                newHeaders[index] = { static_cast<uint32_t>(biomeOffset), old.dataSizeBiome, old.reservedSizeBiome, 
                                      static_cast<uint32_t>(objectOffset), old.dataSizeObject, old.reservedSizeObject };
            }
        }
    }
...
uint8_t ChunkSystem::resolveBiomeId(uint8_t tileIndex) const 
{
    for (const auto& biome : BIOMES) 
    {
        for (const auto& [tile, _] : biome.tileVariants) { if (tile == tileIndex) { return biome.id; } }
    }
    mycerr << "Unknown tileIndex: " << (int)tileIndex;
    return BIOMES[0].id;
}
