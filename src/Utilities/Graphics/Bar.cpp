...
/* Base Bar */
BaseBar::BaseBar(Vector2 position, Vector2 size, Color fill, Color back, std::string name, int fontSize, bool showPercent)
...
/* Timer Bar */
TimerBar::TimerBar(Vector2 position, Vector2 size, float speed, bool isIncreasing, Color fill, Color back, std::string name, int fontSize, bool showPercent)
...
void TimerBar::update(float dt) 
{
    ...
    if (increasing) { progress = std::min(1.0f, progress + delta); }
    else { progress = std::max(0.0f, progress - delta); }
}
...
/* Value Bar */
ValueBar::ValueBar(Vector2 position, Vector2 size, Color fill, Color back, std::string name, int fontSize, bool showPercent)
: BaseBar(position, size, fill, back, name, fontSize, showPercent) {}
